...
public class G 
{
	static class BufferedReader extends Reader {
	    ...
	    @SuppressWarnings("unused")
		StringBuffer read(boolean ignoreLF) throws IOException {	
	    	StringBuffer s = sb;
	    	s.setLength(0);
			int startChar;
		...
		public long nextLong() throws IOException 
		{
			...
			long r = 0;
			int d = 0;
			boolean n = sb.charAt(d) == '-';
			if(n)
				d++;
			r -= sb.charAt(d++) - '0';
			while (d <= sb.length() - 1 && (r *= 10) <= 0) 
				r -= sb.charAt(d++) - '0';
			return n ? r : -r;
		}
	...
	public static void main(String[] args) throws IOException
	{
		...
		for(int i = 0; i < t; i++)
		{
			...
			for(int j = 0; j < n; j++)
			{
				...
				p.add(new Point2D.Double(Integer.parseInt(pedazos[0]), Integer.parseInt(pedazos[1])));
			}
			p.close();
			Point2D pg = p.centerOfMass();
			System.out.println(new BigDecimal(pg.getX() + "").divide(BigDecimal.ONE, 2, BigDecimal.ROUND_HALF_UP) + " " + new BigDecimal(pg.getY() + "").divide(BigDecimal.ONE, 2, BigDecimal.ROUND_HALF_UP));
		}
	}
