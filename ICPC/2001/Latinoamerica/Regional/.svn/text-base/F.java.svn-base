...
public class P6 
{
	...
	public static void main(String[] args)
	{
		...
		while(true)
		{
			...
			if(cuenta > 1)
				System.out.println();
			System.out.println("Problem " + cuenta++ + ":");
			int m = sc.nextInt();
			Polygon este = new Polygon(n);
			for(int i = 0; i < n; i++)
			{
				este.add(new Point2D.Double(sc.nextInt(), sc.nextInt()));
			}
			if(n == 1)
			{
				for(int i = 0; i < m; i++)
					if(este.anterior.equals((new Point2D.Double(sc.nextInt(), sc.nextInt()))))
			...
			else
			{
				este.close();
				for(int i = 0; i < m; i++)
					if(este.contains(new Point2D.Double(sc.nextInt(), sc.nextInt())))
						System.out.println("Within");
					else
						System.out.println("Outside");
			}
